...
{{- define "nautobot.database.engine" -}}
  {{- if (and .Values.postgresql.enabled .Values.postgresqlha.enabled) -}}
...
* The same logic is used for PostgreSQL in HA mode. If you deploy PostgreSQL-HA as part of this chart
  it either takes the values from the existing secret defined in the postgresqlha
  subchart (if defined) or it takes the secret that is generated by the subchart, which
  is done automatically when the subchart is enabled.
